...
struct Flashcards: View {
    ...
    var body: some View {
        ZStack {
            Rectangle()
                .overlay(LinearGradient(gradient: Gradient(colors: [Color(#colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)), Color(#colorLiteral(red: 0.8039215803, green: 0.8039215803, blue: 0.8039215803, alpha: 1))]), startPoint: .topTrailing, endPoint: .bottomLeading))
                .animation(.default)
                .ignoresSafeArea()
        VStack{
            Spacer()
...
struct FlashcardsPreviews: PreviewProvider {
    static var previews: some View {
        Flashcards(word: "Adding", number: 2)
    }
}
